...
\frame{
	...
	\begin{textblock*}{1.09\textwidth}(2.8cm,1.35cm)
		...
		\end{figure}
	\end{textblock*}
	\begin{textblock*}{0.44\textwidth}(-0.2cm,1.55cm)
		\begin{itemize}
			\item Alleviate probability of just having had luck
			\item French Institute for Reseach in Computer Science and Automation (INRIA): Head Pose Image Database
		\end{itemize}
	\end{textblock*}
	\begin{textblock*}{1.2\textwidth}(-0.2cm,5.3cm)
		\begin{itemize}
			\item 15 persons, 2 series per person and 93 images per series
			\item Person changes its head position gradually
			\item Often a person has a certain feature like glasses in one series
			\item Random separation of the 186 images into 149 training and 37 test images
			\item Probably harder problem due to similar colors in all images
		\end{itemize}
	\end{textblock*}
}
